...
class TricepsExercisesScreen extends StatefulWidget {
  const TricepsExercisesScreen({super.key});

  @override
  State<TricepsExercisesScreen> createState() => _TricepsExercisesScreenState();
}
...
class _TricepsExercisesScreenState extends State<TricepsExercisesScreen> {
  ...
  void _loadCustomWorkouts() async {
    ...
    setState(() {
      customWorkouts = prefs.getStringList('customTricepsWorkouts') ?? [];
    });
  }
  ...
  void _loadFavorites() async {
    ...
    setState(() {
      favoriteWorkouts = prefs.getStringList('favoriteTricepsWorkouts') ?? [];
    });
  }
  ...
  void _loadRecent() async {
    ...
    setState(() {
      recentWorkouts = prefs.getStringList('recentTricepsWorkouts') ?? [];
    });
  }
  ...
  void _toggleFavorite(String name) async {
    ...
    await prefs.setStringList('favoriteTricepsWorkouts', favoriteWorkouts);
  }
  ...
  void _addToRecent(String name) async {
    ...
    await prefs.setStringList('recentTricepsWorkouts', recentWorkouts);
  }

  void _addCustomWorkout(String workout) async {
    final cleanWorkout = workout.trim();

    if (cleanWorkout.isEmpty) return;
    if (baseWorkouts.contains(cleanWorkout)) return;
    if (customWorkouts.contains(cleanWorkout)) return;

    final prefs = await SharedPreferences.getInstance();

    setState(() {
      customWorkouts.add(cleanWorkout);
    });

    await prefs.setStringList('customTricepsWorkouts', customWorkouts);
  }
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                setState(() {
                  ...
                  recentWorkouts.remove(workout);
                });

                await prefs.setStringList(
                  'customTricepsWorkouts',
                  customWorkouts,
                );
                await prefs.setStringList(
                  'favoriteTricepsWorkouts',
                  favoriteWorkouts,
                );
                await prefs.setStringList(
                  'recentTricepsWorkouts',
                  recentWorkouts,
                );
